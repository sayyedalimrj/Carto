...
============================
Python Toolbox (ArcMap 10.x, Python 2.7) that exposes the Carto plugin test
suite as a geoprocessing tool, so testers can run it from the ArcMap UI with
an active map document (which also enables Plugin07's AOI_LAYER mode).

Drop this .pyt next to the 'common' and 'tests_arcmap' folders of the
...
class Toolbox(object):
    def __init__(self):
        self.label = "Carto ArcMap Test Harness"
        self.alias = "cartoTestArcMap"
        self.tools = [RunCartoTests]


class RunCartoTests(object):
    def __init__(self):
        self.label = "Run Carto Plugin Tests (ArcMap)"
    ...
    def getParameterInfo(self):
        ...
        p_over = arcpy.Parameter(displayName="Overwrite Existing Test Output",
                                 ...
                                 parameterType="Optional", direction="Input")
        p_over.value = True
        p_mxd = arcpy.Parameter(displayName="MXD Folder (optional, Plugin07 FOLDER_OF_MXDS)",
    ...
    def execute(self, parameters, messages):
        ...
        import arcmap_test_utils as platutils
        ...
        arcpy.AddMessage("Carto ArcMap Test Harness starting...")
        run_dir = core.run(cfg)
        parameters[9].value = run_dir
        arcpy.AddMessage("DONE. Reports: " + os.path.join(run_dir, "reports", "ALL_TEST_SUMMARY.md"))
        return

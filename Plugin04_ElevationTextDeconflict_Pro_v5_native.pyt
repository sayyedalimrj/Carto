# -*- coding: utf-8 -*-
"""
Plugin 04 - Elevation Text Deconflict (ArcGIS Pro / Python 3)  v5 NATIVE
=========================================================================
Native Pro rewrite of the Elevation Text Deconflict tool. Same hardening
philosophy as the ArcMap v5 build (selection bypass, scratchGDB-resident
staging, spatial-indexed obstacle queries via SelectLayerByLocation,
narrow exceptions, [DIAG] logging) but in modern Python 3 with f-strings,
type hints, the arcpy.management/analysis namespaces, and arcpy.mp for
...
def _safe_delete(path: Optional[str], log=None) -> None:
    try:
        ...
        if log:
            log.verbose(f"safe_delete failed for {path}: {traceback.format_exc()}")
        ...
        if self.level != "OFF" and self.path is None:
            try:
                ...
                if sf and os.path.isdir(sf):
                    ...
                    self.path = os.path.join(sf, f"ElevationTextDeconflict_log_{ts}.txt")
            except Exception:
                self.path = None
...
# =============================================================================
# 2. Spatial-indexed obstacle store
# =============================================================================

class _ObstacleStore:
    """
    Wraps obstacle layers as on-disk feature layers with spatial index. Per-
    candidate overlap queries use SelectLayerByLocation against an envelope
    polygon, fetching ONLY the touching obstacles.
    """
    __slots__ = ("layers", "log")

    def __init__(self, log: Logger):
        self.layers: List[Tuple[str, str]] = []  # (lyr_name, fc_path)
        self.log = log
    ...
    def add_layer(self, layer_or_path) -> None:
        ...
        if not src or not arcpy.Exists(src):
            if self.log:
                self.log.warn(f"OBSTACLE: layer does not exist: {layer_or_path}")
            return
        try:
            try:
            ...
            try:
                ...
                n = -1
            if self.log:
                self.log.diag(f"OBSTACLE: '{src}' total={n}")
        except Exception:
            if self.log:
                self.log.warn(
                    f"OBSTACLE: MakeFeatureLayer failed for {layer_or_path}: "
    ...
    def conflict_in_box(self, sr, cx: float, cy: float,
        ...
        for (lyr_name, _) in self.layers:
            try:
                ...
                try:
                    sel_count = int(arcpy.management.GetCount(lyr_name).getOutput(0))
                except Exception:
                    sel_count = 0
                if sel_count <= 0:
                    continue
                ...
                if self.log:
                    self.log.verbose(
                        f"conflict_in_box query failed: {traceback.format_exc()}")
                continue
        return False


# =============================================================================
# 3. AABB cache for placed labels (Mode A self-overlap)
# =============================================================================
...
class _PlacedCache:
    ...
    def conflicts(self, foot_geom) -> bool:
...
def _iter_candidates(pattern: Optional[str], rings_sorted: List[float],
                     angles: List[float], max_ring: float, spiral_step: float):
...
def _rect_polygon_rotated(sr, cx: float, cy: float, w: float, h: float,
    ...
    try:
        ...
        pts = [arcpy.Point(cx + (x * ca - y * sa), cy + (x * sa + y * ca))
               for (x, y) in corners]
        return arcpy.Polygon(arcpy.Array(pts), sr)
    except Exception:
        return _rect_polygon(sr, cx, cy, w, h)

def _text_extent_map_units(text, font_size_pt: float, k: float,
...
def _ensure_field(fc: str, name: str, ftype: str, length: Optional[int] = None) -> None:
...
# =============================================================================
# 5. Toolbox + Tool class (parameters)
# =============================================================================

class Toolbox:
    def __init__(self):
        self.label = "Plugin 4 - Elevation Text Deconflict (Pro, v5 native)"
        self.alias = "elevtext_v5_pro"
        self.tools = [ElevationTextDeconflictV5]


class ElevationTextDeconflictV5:
    def __init__(self):
        self.label = "Elevation Text Deconflict (2 Modes) - v5 native"
        self.description = (
    ...
    def getParameterInfo(self):
        ...
        params.append(p6)
        p7 = arcpy.Parameter(
            displayName="(Mode B) Max Anchor Match Distance (map units) [blank=no limit]",
        ...
        params.append(p11)
        p12 = arcpy.Parameter(
            displayName="Max Features per Obstacle Layer (deprecated; ignored in v5)",
        ...
        p15.value = 25000
        params.append(p15)
        p16 = arcpy.Parameter(displayName="(Mode A) Font Size (pt)",
        ...
        p26.value = False
        params.append(p26)
        p_addmap = arcpy.Parameter(displayName="Add outputs to current map",
    ...
    def updateParameters(self, parameters):
        ...
        modeB_names = {"anno_layer", "anchor_points", "link_method", "featureid_field",
    ...
    # ----- Pro: helper to push outputs into active map -----
    ...
    def _execute_mode_a(self, log: Logger, in_points_layer, text_field: str,
                        ...
                        obstacle_store: _ObstacleStore, base_sr,
        ...
        with arcpy.da.SearchCursor(
                in_points_path, ["OID@", "SHAPE@", text_field]) as sc, \
            ...
            for oid, geom, txt in sc:
                ...
                if placed:
                    ...
                    report_bad_rows.append((p0, oid,
                        ...
                        "max attempts exceeded", oid, txt_r))
                if (total_in % 500) == 0:
                    gc.collect()

        log.diag(
            f"MODE A: total={total_in} moved={moved} unchanged={unchanged} "
            f"failed={failed}")
        ...
        if create_moved_only:
            ...
            try:
                ...
                log.warn(f"MODE A: moved-only output failed: {traceback.format_exc()}")
                moved_only_final = None
            finally:
                try:
                    arcpy.management.Delete(lyr_name)
        ...
        if create_leaderlines:
            try:
                ...
                log.warn(f"MODE A: leader lines output failed: {traceback.format_exc()}")
                leaderlines_fc = None
    ...
    # =========================================================================
    # 5.2 Mode B: anchor map (on-disk staging via scratchGDB)
    # =========================================================================

    def _create_temp_polygon_from_annotation(self, log: Logger, anno_fc: str,
                                              base_sr, scratch_gdb: str):
        ...
        log.diag(f"MODE B: Anno-as-polygon FC built. rows={inserted} path={temp_fc}")
        return temp_fc, poly2anno

    def _build_anchor_map_modeB(self, log: Logger, anno_fc: str, anchor_points,
                                 link_method: str, featureid_field: str,
                                 max_match_dist_text, base_sr, scratch_gdb: str):
        ...
        if link_method == "FEATUREID_MATCH":
            ...
            log.diag(f"MODE B: Anchor map (FEATUREID_MATCH) matched={len(mapping)}")
            return mapping
        ...
        try:
            ...
            with arcpy.da.SearchCursor(
                ...
                for in_fid, near_fid, near_dist in tc:
                    ...
                    matched += 1
            log.diag(f"MODE B: Anchor map (NEAREST_POINT) matched={matched}")
            return mapping
        finally:
    ...
    def _execute_mode_b(self, log: Logger, anno_layer, anchor_points, link_method,
                        ...
                        obstacle_store: _ObstacleStore, base_sr, scratch_gdb,
        ...
        stamp = datetime.datetime.now().strftime("%H%M%S")
        moved_copy_final = os.path.join(out_container, f"Annotation_Moved_{stamp}")
        moved_only_final = os.path.join(out_container, f"Annotation_MovedOnly_{stamp}")
        ...
        if not workspace:
            raise arcpy.ExecuteError(
                f"Could not determine file geodatabase path from annotation: "
                f"{original_workspace}")
        ...
        try:
            ...
            with arcpy.da.UpdateCursor(moved_copy_final, u_fields) as ucur:
                for aoid, ageom, textstr, xo, yo, mv, st, rr, aa, att, ao, ad in ucur:
                    ...
                    if placed:
                        ...
                        c = ageom.centroid
                        p = arcpy.PointGeometry(arcpy.Point(c.X, c.Y), base_sr)
                        report_all_rows.append((p, aoid, "FAILED", 0.0, 0.0,
            ...
            log.error(f"MODE B: edit session failed: {traceback.format_exc()}")
            try:
        ...
        log.diag(
            ...
            f"moved={moved} unchanged={unchanged} failed={failed} skipped={skipped}")

        # Optional rotation write (Pro: PYTHON3)
            ...
            try:
                ...
                if angle_field:
                    ...
                    if rotation_write_mode == "ADD_DELTA":
                        ...
                        arcpy.management.CalculateField(
                            lyr_rot, angle_field, "!ETD_ANGLE!", "PYTHON3")
                    arcpy.management.Delete(lyr_rot)
            except Exception:
                log.warn(f"MODE B: rotation write failed: {traceback.format_exc()}")

        # Force redraw / dirty flag (still helps in Pro)
        try:
            arcpy.management.CalculateField(
                moved_copy_final, "ETD_MOVED", "!ETD_MOVED!", "PYTHON3")
        except Exception:
            pass
        try:
        ...
        if moved_only_final:
            try:
                ...
                log.warn(
                    "MODE B: FeatureClassToFeatureClass failed; using fallback.")
        ...
        if create_modeB_points:
            try:
                ...
                log.warn(
                    f"MODE B: label-position points failed: {traceback.format_exc()}")
                label_positions_points = None
        ...
        if create_leaderlines:
            try:
                ...
                log.warn(f"MODE B: leader lines failed: {traceback.format_exc()}")
                leaderlines_fc = None
    ...
    def execute(self, parameters, messages):
        ...
        def _p_bool(name, default=False):
            ...
            if v is None:
                return default
            try:
                return str(v).strip().lower() in ("true", "t", "1", "yes", "y", "on")
            except Exception:
                return default
        ...
        if not is_empty_gp(obstacle_layers_txt):
            obstacle_layers = [s for s in _safe_str(obstacle_layers_txt).split(";")
                                if s.strip()]
        ...
        try:
            ...
            log.error(f"Exception: {_safe_str(e)}")
        ...
        # Pro: optionally add outputs to active map
        if add_to_map:
            paths = [moved_copy_final, moved_only_final, leaderlines_fc,
                     label_positions_points if (label_positions_points
                                                and label_positions_points
                                                != moved_copy_final) else None,
                     out_all, out_bad]
        ...
        if mode == "ANNOTATION_LAYER_AND_ANCHOR_POINTS":
            log.info("Mode B: movement applied via XOffset/YOffset on the output "
                     "copy. Input is not modified.")
        else:
            log.info("Mode A: input points are NOT modified. Output is label positions.")
        log.info(f"Outputs: moved_copy={moved_copy_final} moved_only={moved_only_final}")

# -*- coding: utf-8 -*-
"""
Plugin 05 - Safe Contour Cleaner (ArcGIS Pro / Python 3)  v5 NATIVE
====================================================================
Native Pro rewrite. Same hardening philosophy as the ArcMap v5 build
(selection bypass, scratchGDB-resident staging, chunked Near table,
narrow exceptions, [DIAG] logging) in modern Python 3 with f-strings,
type hints, the arcpy.management/analysis namespaces, and arcpy.mp
for map integration.

Two tools:
  Tool 1: AOI Brush Builder
    Build an AOI polygon from polyline/polygon brush strokes via
    buffer + union/erase, optionally clipped to a frame.

  Tool 2: Safe Contour Cleaner (Print-Ready)
    Build a cleaned COPY of contour line layers for cartographic
    output, leaving the originals untouched. Removes dense /
    overlapping segments inside an AOI while protecting a safety
    band inside the frame border. Supports segment erase or
    whole-feature delete, optional min-segment-length cleanup,
...
# =============================================================================
# 3. Pro: scale + map integration helpers
# =============================================================================

def _get_active_map_scale() -> Optional[float]:
    """Return the active map view's scale if running inside Pro."""
    try:
        ...
        try:
            ...
            if op in (
                ...
                parameters[4].setWarningMessage(
                    "Operation requires an Existing AOI polygon. Provide it "
                    "or switch to 'Create new AOI'.")
        except Exception:
            pass
        ...
        try:
            ...
            if brush_radius_mm and brush_radius_mm > 0:
                ...
                if conv is not None:
                    ...
                    _msg(f"Brush radius converted from mm using scale 1:{scale:.0f} => {conv:.3f} map units")
                else:
                    _warn("Could not convert brush radius (mm). Using map-units value only.")
            ...
            if brush_geom_type in ("POLYLINE", "LINE", "POINT", "MULTIPOINT"):
                if brush_radius_mu <= 0:
                    raise arcpy.ExecuteError(
                        "Brush Radius must be > 0 for Polyline/Point brush strokes.")
            ...
            if clip_to_frame and frame_fc:
                ...
                try:
                    ...
                    _warn(f"Clip to frame failed; using unclipped AOI. {traceback.format_exc()}")
...
class SafeContourCleaner:
    ...
    def _clip_like(self, in_fc, clip_fc, out_fc):
        try:
            ...
            _warn("Clip failed; falling back to SelectByLocation + CopyFeatures.")
    ...
    def _safe_zone_from_frame(self, frame_fc, safe_margin, out_safe_zone_fc):
        ...
        try:
            ...
            _warn("Buffer failed for safe zone; using full frame as safe zone.")
            arcpy.management.CopyFeatures(dissolved, out_safe_zone_fc)
        ...
        try:
            ...
            _warn("Clip failed for safe zone; using full frame as safe zone.")
            arcpy.management.CopyFeatures(dissolved, out_safe_zone_fc)
        _safe_delete(dissolved); _safe_delete(frame_line); _safe_delete(band)
        return out_safe_zone_fc
        ...
        if count <= near_chunk:
            ...
            while i < n:
                ...
                try:
                    ...
                    _warn(f"Near chunk {chunk_idx} failed: {traceback.format_exc()}")
                finally:
                    _safe_delete(near_tbl)
                    try:
    ...
    def _report_csv(self, out_ws, base_name,
        ...
        with open(report_path, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["Tool", "Safe Contour Cleaner (v5 native)"])
    ...
    def getParameterInfo(self):
        ...
        p22.description = (
            "For very large eligible sets, GenerateNearTable runs in OID-bounded "
            "chunks of this size.")
        params.append(p22)
        ...
        try:
            ...
            if aoi_mode and "Frame" in aoi_mode and not frame:
                parameters[self.IDX_FRAME_POLY].setWarningMessage(
                    ...
                    "provided. Tool will fall back to active map view extent.")
    ...
    def execute(self, parameters, messages):
        ...
        try:
            ...
            custom_aoi_fc = (_resolve_full_source(custom_aoi_layer)
                             if custom_aoi_layer else None)
            external_mask_fc = (_resolve_full_source(external_mask_layer)
            ...
            if safe_mm and safe_mm > 0:
                ...
                if conv is not None:
                    _msg(f"Safe Margin converted from {safe_mm} mm @ 1:{scale:.0f} => {conv:.3f} map units")
                    safe_mu = max(safe_mu, conv)
                else:
                    _warn("Could not convert Safe Margin (mm). Using map-units value only.")

            # Frame fallback: active map view extent (Pro)
            if (not frame_fc) and aoi_mode and "Frame" in aoi_mode:
                try:
                    aprx = arcpy.mp.ArcGISProject("CURRENT")
                    mv = aprx.activeView
                    cam = getattr(mv, "camera", None)
                    ext = getattr(cam, "getExtent", None)
                    if ext is not None:
                        ext = ext()
                    if not ext:
                    ...
                    _warn("Frame polygon not provided. Using active map view extent as frame.")
                except Exception:
                    raise arcpy.ExecuteError(
                        "Frame polygon is required OR run inside Pro with an active map view.")
            ...
            if aoi_mode == "Frame only (default)":
                ...
                if not custom_aoi_fc:
                    raise arcpy.ExecuteError(
                        "Custom AOI mode selected but Custom AOI polygon not provided.")
                aoi_fc = custom_aoi_fc
            elif aoi_mode == "Frame AND Custom AOI":
                if not (frame_fc and custom_aoi_fc):
                    raise arcpy.ExecuteError(
                        "Frame AND Custom AOI requires BOTH Frame polygon and Custom AOI polygon.")
            ...
            _diag(f"Eligible: {eligible_count} | Protected/Other: {noneligible_count}")
            ...
            if mask_mode == "Use external mask polygon":
                if not external_mask_fc:
                    raise arcpy.ExecuteError(
                        "Mask Mode is 'Use external mask polygon' but no external "
                        "mask was provided.")
                arcpy.management.CopyFeatures(external_mask_fc, mask_fc)
            elif mask_mode == "Use AOI polygon as mask (no auto)":
                if not aoi_fc:
                    raise arcpy.ExecuteError(
                        "AOI is required to use AOI as mask (choose a Frame or Custom AOI mode).")
            ...
            if mask_count == 0:
                _warn("Mask is empty. No contour segments will be removed (output becomes copy).")
            ...
            if (safe_zone_fc and mask_count > 0
                ...
                try:
                    ...
                    _warn("Erase failed while subtracting Safe Zone from mask. Using raw mask.")
                    _safe_delete(final_mask_fc)
                    final_mask_fc = mask_fc

            final_mask_count = _get_count(final_mask_fc)
            _diag(f"Final mask polygons after frame safety: {final_mask_count}")
            if final_mask_count == 0 and mask_count > 0:
                _warn("After frame safety, final mask is empty (all dense zones were within Safe Margin).")
            ...
            if create_review and safe_zone_fc and mask_count > 0:
                ...
                try:
                    ...
                    _warn("Could not build review intersect.")
            ...
            if dry_run:
                ...
                _diag(f"DRY RUN done. inputs={total_in} eligible={eligible_count} mask={mask_count} final_mask={final_mask_count}")
                if add_to_map:
            ...
            if removal_method == "Segment Erase (recommended)":
                try:
                    ...
                    prod = None
                if prod != "ArcInfo":
                    _warn("Advanced license not detected (Erase may fail). Falling back to 'Delete Whole Features'.")
            ...
            _diag(f"Output kept count: {kept_count}, approx removed length: {removed_len_approx:.2f}")
            ...
            if write_report:
                try:
                    ...
                    _warn(f"Failed to write CSV report. {traceback.format_exc()}")
